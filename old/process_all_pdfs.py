...
def clean_and_convert_to_markdown(text, title):
    ...
    while i < len(cleaned_lines):
        ...
        if re.match(r'CAPÍTULO\s+[IVX]+', line) or re.match(r'APÉNDICE\s+', line):
...
def process_all_pdfs():
    # Directorio de apéndices financieros
    financieros_dir = "0.0 contrrato en pdf/Apéndices Financieros"
    
    # Obtener todos los archivos PDF
    pdf_files = []
    for file in os.listdir(financieros_dir):
        if file.endswith('.pdf'):
            pdf_files.append(os.path.join(financieros_dir, file))
    
    # Procesar cada PDF
    all_content = []
    all_content.append("# APÉNDICES FINANCIEROS - CONTRATO DE CONCESIÓN")
    all_content.append("")
    all_content.append("---")
    all_content.append("")
    ...
    output_file = "apendices_financieros.md"
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write('\n'.join(all_content))
    
    print(f"\nTodos los apéndices financieros convertidos a: {output_file}")
